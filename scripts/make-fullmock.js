...
function main() {
  ...
  const mathsId = args.math || 'ssc_cgl_12_sep_s2';
  const reasoningId = args.reasoning || 'ssc_cgl_12_sep_s2-r';
  const englishId = args.english || 'ssc_cgl_eng_12_sep_s2';
  const gkId = args.gk || 'ssc_cgl_gk_12_sep_s2';

  // Output test ID (single full mock of 100 questions)
  const OUTPUT_ID = args.out || 'ssc_cgl_fullmock_12_sep_s2';
